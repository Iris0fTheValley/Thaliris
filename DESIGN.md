...
Roles name responsibilities; current recommendations are Controller/Control Plane, Investigator, and Curator -> GPT-5.6 Luna; Reasoning Specialist -> GPT-5.6 Sol; Implementer and Independent Reviewer -> GPT-5.6 Terra. The Reasoning Specialist receives only controller-promoted Decision Context plus routed durable constraints and decisions. It has no task-state write permission: requests for more facts or verification return to the Controller as reasoning output. An Investigator appends task-local raw findings and evidence; a fresh Curator rewrites only the compact investigation snapshot. An Implementer receives the explicit modification boundary and changed surface. An Independent Reviewer receives only intent, hard constraints, durable decisions, changed surface/diff, and the evidence necessary to inspect them; reviewer findings remain outside both its input pack and the Reasoning Specialist until the Controller promotes one. The persistent Controller retains the complete routing view and is the only role that modifies Decision Context. A microtask still follows Controller -> Implementer -> deterministic verification -> done; it does not authorize Controller direct source edits.
