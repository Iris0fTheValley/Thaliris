...
successful child result. `interrupted`, `errored`, and `shutdown` remain failed
even if a later Stop hook is observed. `not_found` is `ORPHANED`, not completion. Repeated
