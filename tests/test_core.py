...
def test_controller_packet_is_bounded(tmp_path: Path) -> None:
    ...
    # Core is runtime-neutral; managed Codex start readiness is tested at the
    # adapter boundary rather than by this Core packet test.
    assert core.task_start(root, "bounded task", None, None)["ok"] is True
...
def test_artifact_registration_is_explicit_and_missing_is_loadable(tmp_path: Path) -> None:
    ...
    core.task_start(root, "artifact", None, None)
